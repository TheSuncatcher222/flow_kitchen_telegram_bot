...
class IsDeveloper(Filter):
    """
    Фильтр доступа для разработчиков.
    """

    async def __call__(self, message: Message) -> bool:
        return check_if_user_is_developer(user_id_telegram=message.from_user.id)
...
def check_if_user_is_developer(user_id_telegram: int | str) -> bool:
    """
    Проверяет, является ли пользователь разработчиком.
    """
    return str(user_id_telegram) in settings.BOT_DEVELOPER_IDS
